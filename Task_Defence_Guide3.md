...
When a user (or Postman) sends a request to `http://localhost:8083/students`:

1. **The Request hits `Task3Application.java`**: Spring Boot receives the request.
...
We organized the codebase into a clean, modular structure under `lt.viko.eif.imran.Task3`:
- **`config`**: Holds configuration classes (e.g., `JerseyConfig.java`) that tell the framework how to register our JAX-RS endpoints.
- **`model`**: Contains the data objects (`Student.java`, `Subject.java`) representing our core entities.
- **`resource`**: Contains the REST endpoints (`StudentResource.java`) which handle web HTTP requests.
- **`service`**: Holds the core business logic (`StudentService.java`, `StudentServiceImpl.java`), ensuring our endpoints stay lightweight and clean.

---

## 4. S.O.L.I.D. Principles Explained
...
## 5. Master Your Annotations (CRITICAL)

Your teacher will definitely ask you about the `@` symbols (annotations) in your code. Here is your cheat sheet:

### Spring Boot Annotations
- **`@SpringBootApplication`** *(in Task3Application.java)*: This tells Java "This is a Spring Boot app, configure everything automatically and start the server here."
...
### Method A: Using your IDE (IntelliJ IDEA / Eclipse)
1. Open your IDE.
2. Click **File -> Open** and select the `Task3` folder.
3. Your IDE will recognize it as a Maven project and download dependencies.
4. Open `Task3Application.java` and click the green "Run" button.
5. The console will say Tomcat started on port 8083.

### Method B: Using the Command Line (Terminal / PowerShell)
This is a great way to impress your teacher during the defense!
1. Open your terminal or PowerShell and navigate to the `Task3` folder.
2. Run the unit tests to prove the code works: 
   - Windows: `.\mvnw.cmd clean test`
   - Mac/Linux: `./mvnw clean test`
3. Start the application: 
   - Windows: `.\mvnw.cmd spring-boot:run`
   - Mac/Linux: `./mvnw spring-boot:run`
4. The console will confirm Tomcat started on port 8083.

---

## 7. How to Test (SoapUI, Postman, Swagger)
...
- **Method**: `GET`
- **URL**: `http://localhost:8083/students/1`
...
### OpenAPI & Swagger (Modern Generation)

Since third-party tools like APIMATIC now require business email signups, we configured your application to generate the OpenAPI JSON standard natively! This is the professional, modern way to do it.

**Where and How is it Generated in the Code?**
If your teacher asks how you generated the OpenAPI JSON without APIMATIC, tell them:
1. **In `pom.xml`**: We added the `swagger-jaxrs2-jakarta` library.
2. **In `JerseyConfig.java`**: We registered `OpenApiResource.class`.
3. **The Magic**: Swagger automatically scans your `StudentResource.java` file while the app runs. It reads your annotations (`@Path`, `@GET`, `@POST`) and looks at your `Student` model, automatically writing the OpenAPI JSON map for you behind the scenes!

**How to view and test it:**
1. Start your application in IntelliJ (make sure you click **Reload All Maven Projects** first!).
2. Go to `http://localhost:8083/openapi.json` in your browser.
3. Copy the raw JSON text you see on that page.
4. Go to **Swagger Editor** (https://editor.swagger.io/) and paste the JSON on the left side.
5. You will instantly see a beautiful visual map of your API on the right side, where you can click buttons to test your endpoints!
